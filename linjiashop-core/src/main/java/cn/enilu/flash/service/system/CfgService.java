...
//        return configCache.get(cfgName);
    ...
    /**
     * 判断当前配置与数据库已有配置的参数名是否重复
     *
     * @param cfg
     * @return
     */
    public Boolean isRepeat(Cfg cfg) {
        Cfg old = null;
        if (cfg.getId() != null) {
            old = get(
                    Lists.newArrayList(SearchFilter.build("cfgName", cfg.getCfgName()),
                            SearchFilter.build("id", SearchFilter.Operator.NE, cfg.getId())
                    ));
        } else {
            old = get(SearchFilter.build("cfgName", cfg.getCfgName()));
        }
        return old != null;
    }
}
